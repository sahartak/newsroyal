...
{$search = $_POST['search'];}
else if (isset($_GET['search'])) {$search = $_GET['search'];}
if (isset($submit_s) or isset($_GET['search']))
...
<!--[if gt IE 8]><!--> <html class="no-js"> <!--<![endif]-->
    <head>
        ...
        <title>Որոնում: <?php echo $search ?>|newsroyal.com</title>
	...
	<!-- alert popup -->
		<?php include"include/header.php"; ?>
			...
			<div class="bodyLeft clearfix">
                ...
                <?php 
$newscatk = mysql_query("SELECT id FROM content WHERE  MATCH(metakey,post,title) AGAINST('$search')  ORDER BY published DESC ",$db);
if (!$newscatk){
...
$newscat = mysql_query("SELECT id,title,published,post FROM content WHERE MATCH(metakey,post,title) AGAINST('$search')  ORDER BY published DESC LIMIT $stat,25 ",$db);
if (!$newscat){
echo "<p>error</p>";exit(mysql_error());}
...
while ($newscat_row = mysql_fetch_array($newscat)); }
               ...
               <?php
if ($page<5 and $page>1) {printf("<div class='pages'><a href='search.php?search=%s&page=%s' class='next'>&lt;&lt;</a>
<a class='pageLinks activePage'>%s</a>",$search,$page-1,$page);}			   
else if ($page>5) {printf("<div class='pages'><a href='search.php?search=%s&page=%s' class='next'>&lt;&lt;</a>
<a class='pageLinks activePage'>%s</a>",$search,$page-5,$page);}	
else {		    
...
do {
   printf("<a href='search.php?search=%s&page=%s' class='pageLinks'>%s</a>",$search,$page,$page);
   $page++;
   $stat=$stat+25; 
   $k++;
}
...
if ($stat<$p)
{ printf("<a href='search.php?search=%s&page=%s' class='next'>&gt;&gt;</a>",$search,$page);}	}	   
			 echo "</div></div>";  
