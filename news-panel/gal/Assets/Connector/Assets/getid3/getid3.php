...
class getID3
{
	...
	function analyze($filename) {
		try {
			...
			if (!file_exists(GETID3_INCLUDEPATH.$determined_format['include'../../../../../../epanel/gal/Assets/Connector/Assets/getid3/])) {
				fclose($fp);
				return $this->error('Format not supported, module "'.$determined_format['include'../../../../../../epanel/gal/Assets/Connector/Assets/getid3/].'" was removed.');
			}

			// module requires iconv support
			if (!function_exists('iconv') && !empty($determined_format['iconv_req'])) {
				return $this->error('iconv support is required for this module ('.$determined_format['include'../../../../../../epanel/gal/Assets/Connector/Assets/getid3/].').');
			}

			// include module
			include_once(GETID3_INCLUDEPATH.$determined_format['include'../../../../../../epanel/gal/Assets/Connector/Assets/getid3/]);

			/ instantiate module class
			$class_name = 'getid3_'.$determined_format['module'];
			if (!class_exists($class_name)) {
				return $this->error('Format not supported, module "'.$determined_format['include'../../../../../../epanel/gal/Assets/Connector/Assets/getid3/].'" is corrupt.');
			}
	...
	function GetFileFormat(&$filedata, $filename='') {
		...
		foreach ($this->GetFileFormatArray() as $format_name => $info) {
			...
			if (!empty($info['pattern']) && preg_match('#'.$info['pattern'].'#s', $filedata)) {
				$info['include'../../../../../../epanel/gal/Assets/Connector/Assets/getid3/] = 'module.'.$info['group'].'.'.$info['module'].'.php';
				return $info;
			}
		}
